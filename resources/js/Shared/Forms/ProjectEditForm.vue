...
const update = () => {
    router.put(`/projects/${props.project.id}?keyword=${search.value}`, form);
};
...
const clientWebsites = computed(() => {
    return form.client ? clients.value.find(client => client.id === form.client).websites : [];
});
...
<template>
    <Form @submit.prevent="update">
        ...
        <Select v-model="form.client" :error="errors.client" label="Client">
